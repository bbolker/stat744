...
* homework is due Wed at noon (the week after it is assigned).
...
* Find a table (preferably a complicated one) and turn it into a graph. One option would be Table 3 of @mccallum_situ_2017; you can find most of (an earlier version of) the data for this graph [here](https://github.com/bbolker/stat744/blob/master/data/POCIS_Raw_McCallum.csv) and a table that gives the correspondences between drug abbreviations and full names as well as drug categories  [here](https://github.com/bbolker/stat744/blob/master/data/drugnames.csv).

If you have read in the data as `dd` and the abbreviation table as `dn`, code like this will convert the data to long form and add the abbrevations ...
